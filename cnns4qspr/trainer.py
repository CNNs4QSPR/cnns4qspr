...
from cnns4qspr.se3cnn_v3.util.losses import vae_loss, predictor_loss
...
class VAETrainer():
    ...
    def train(self,
        ...
        latent_means_train = np.zeros((n_epochs, self.n_train_samples, self.network.latent_size))
        latent_means_val = np.zeros((n_epochs, self.n_val_samples, self.network.latent_size))
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, data in enumerate(train_loader):
                ...
                pred_l = predictor_loss(label, prediction)
                loss = vae_l + pred_l
                loss.backward()
                self.optimizer.step()
                self.optimizer.zero_grad()
                ...
                train_accs.append(acc.data.cpu().numpy())
